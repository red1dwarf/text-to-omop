...
UPDATE omop.concept
SET 
  load_table_id = 'concept',
  load_row_id = ('x' || substr(md5(gen_random_uuid()::text), 1, 16))::bit(64)::bigint;

SELECT load_row_id, COUNT(*) AS occurrences
FROM omop.concept
GROUP BY load_row_id
HAVING COUNT(*) > 1;

ALTER TABLE omop.concept_relationship
ADD COLUMN load_table_id TEXT DEFAULT 'concept_relationship',
ADD COLUMN load_row_id INTEGER DEFAULT 0;

ALTER TABLE omop.vocabulary
ADD COLUMN load_table_id TEXT DEFAULT 'vocabulary',
ADD COLUMN load_row_id INTEGER DEFAULT 0;

ALTER TABLE omop.drug_strength
ADD COLUMN load_table_id TEXT DEFAULT 'drug_strength',
ADD COLUMN load_row_id INTEGER DEFAULT 0;

ALTER TABLE omop.concept_class
ADD COLUMN load_table_id TEXT DEFAULT 'concept_class',
ADD COLUMN load_row_id INTEGER DEFAULT 0;

ALTER TABLE omop.concept_ancestor
ADD COLUMN load_table_id TEXT DEFAULT 'concept_ancestor',
ADD COLUMN load_row_id INTEGER DEFAULT 0;

ALTER TABLE omop.concept_synonym
ADD COLUMN load_table_id TEXT DEFAULT 'concept_synonym',
ADD COLUMN load_row_id INTEGER DEFAULT 0;

ALTER TABLE omop.domain
ADD COLUMN load_table_id TEXT DEFAULT 'domain',
ADD COLUMN load_row_id INTEGER DEFAULT 0;

ALTER TABLE omop.relationship
ADD COLUMN load_table_id TEXT DEFAULT 'relationship',
ADD COLUMN load_row_id INTEGER DEFAULT 0;