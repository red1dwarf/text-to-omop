...
DROP TABLE IF EXISTS omop.lk_micro_cross_ref;
CREATE TABLE omop.lk_micro_cross_ref AS
SELECT
    trace_id AS trace_id_ab, -- for antibiotics
    FIRST_VALUE(src.trace_id) OVER (
        PARTITION BY
    ...
    subject_id, -- to pick additional hadm_id from admissions
    hadm_id,
    src.order_provider_id AS order_provider_id,
    COALESCE(src.charttime, src.chartdate) AS start_datetime, -- just to do coalesce once
    COALESCE(src.storetime, src.storedate) AS end_datetime
FROM
    omop.src_microbiologyevents src -- mbe
;



DROP TABLE IF EXISTS omop.lk_micro_hadm_id;
CREATE TABLE omop.lk_micro_hadm_id AS
SELECT
    src.trace_id_ab AS event_trace_id,
    adm.hadm_id AS hadm_id,
    ROW_NUMBER() OVER (
...
FROM  
    omop.lk_micro_cross_ref src
INNER JOIN 
    omop.lk_admissions_clean adm
        ON adm.subject_id = src.subject_id
        AND src.start_datetime BETWEEN adm.start_datetime AND adm.end_datetime
WHERE
    src.hadm_id IS NULL
...
DROP TABLE IF EXISTS omop.lk_meas_organism_clean;
CREATE TABLE omop.lk_meas_organism_clean AS
SELECT DISTINCT
    src.subject_id AS subject_id,
    src.hadm_id AS hadm_id,
    ...
    cr.trace_id_org AS trace_id -- trace_id for test-organism
FROM
    omop.src_microbiologyevents src
INNER JOIN
    omop.lk_micro_cross_ref cr
        ON src.trace_id = cr.trace_id_org
;



DROP TABLE IF EXISTS omop.lk_specimen_clean;
CREATE TABLE omop.lk_specimen_clean AS
SELECT DISTINCT
    src.subject_id AS subject_id,
    src.hadm_id AS hadm_id,
    ...
    cr.trace_id_spec AS trace_id -- trace_id for specimen
FROM
    omop.lk_meas_organism_clean src -- mbe
INNER JOIN
    omop.lk_micro_cross_ref cr
        ON src.trace_id = cr.trace_id_spec
;
...
DROP TABLE IF EXISTS omop.lk_meas_ab_clean;
CREATE TABLE omop.lk_meas_ab_clean AS
SELECT
    src.subject_id AS subject_id,
    src.hadm_id AS hadm_id,
    cr.order_provider_id AS provider_id,
...
DROP TABLE IF EXISTS omop.lk_d_micro_clean;
CREATE TABLE omop.lk_d_micro_clean AS
SELECT
    dm.itemid AS itemid,
...
DROP TABLE IF EXISTS omop.lk_d_micro_concept;
CREATE TABLE omop.lk_d_micro_concept AS
SELECT
    dm.itemid AS itemid,
    dm.source_code AS source_code, -- itemid
    dm.source_label AS source_label, -- symbolic information in case more mapping is required
...
DROP TABLE IF EXISTS omop.lk_specimen_mapped;
CREATE TABLE omop.lk_specimen_mapped AS
SELECT
    ('x' || substr(md5(gen_random_uuid()::text), 1, 16))::bit(64)::bigint AS specimen_id,
    src.subject_id AS subject_id,
    COALESCE(src.hadm_id, hadm.hadm_id) AS hadm_id,
    CAST(src.start_datetime AS DATE) AS date_id,
    32856 AS type_concept_id, -- Lab
    ...
    src.trace_id AS trace_id
FROM
    omop.lk_specimen_clean src
INNER JOIN
    omop.lk_d_micro_concept mc
        ON src.spec_itemid = mc.itemid
LEFT JOIN
    omop.lk_micro_hadm_id hadm
        ON hadm.event_trace_id = src.trace_id
        AND hadm.row_num = 1
;
...
DROP TABLE IF EXISTS omop.lk_meas_organism_mapped;
CREATE TABLE omop.lk_meas_organism_mapped AS
SELECT
    ...
    src.trace_id AS trace_id
FROM
    omop.lk_meas_organism_clean src
INNER JOIN
    omop.lk_d_micro_concept tc
        ON src.test_itemid = tc.itemid
INNER JOIN
    omop.lk_d_micro_concept sc
        ON src.spec_itemid = sc.itemid
LEFT JOIN
    omop.lk_d_micro_concept oc
        ON src.org_itemid = oc.itemid
LEFT JOIN
    omop.lk_micro_hadm_id hadm
        ON hadm.event_trace_id = src.trace_id
...
DROP TABLE IF EXISTS omop.lk_meas_ab_mapped;
CREATE TABLE omop.lk_meas_ab_mapped AS
SELECT
    ('x' || substr(md5(gen_random_uuid()::text), 1, 16))::bit(64)::bigint AS measurement_id,
    src.subject_id AS subject_id,
    COALESCE(src.hadm_id, hadm.hadm_id) AS hadm_id,
...
FROM
    omop.lk_meas_ab_clean src
INNER JOIN
    omop.lk_d_micro_concept ac
        ON src.ab_itemid = ac.itemid
LEFT JOIN
    omop.lk_d_micro_concept rc
        ON src.interpretation = rc.source_code
        AND rc.source_vocabulary_id = 'mimiciv_micro_resistance'
LEFT JOIN
    omop.lk_meas_operator_concept opc
        ON src.dilution_comparison = opc.source_code
LEFT JOIN
    omop.lk_micro_hadm_id hadm
        ON hadm.event_trace_id = src.trace_id
