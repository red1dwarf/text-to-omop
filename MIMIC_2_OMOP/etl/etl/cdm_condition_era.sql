...
DROP TABLE IF EXISTS omop.tmp_target_condition;
CREATE TABLE omop.tmp_target_condition AS
SELECT
    co.condition_occurrence_id AS condition_occurrence_id,
    co.person_id AS person_id,
    co.condition_concept_id AS condition_concept_id,
    ...
    -- - to set condition_era_end_date to same condition_era_start_date
          -- or condition_era_start_date + INTERVAL '1 day', when condition_end_date IS NULL
FROM
    omop.cdm_condition_occurrence co
WHERE
    co.condition_concept_id != 0
;

DROP TABLE IF EXISTS omop.tmp_dates_un_condition;
CREATE TABLE omop.tmp_dates_un_condition AS
SELECT
    person_id AS person_id,
    condition_concept_id AS condition_concept_id,
    condition_start_date AS event_date,
...
DROP TABLE IF EXISTS omop.tmp_dates_rows_condition;
CREATE TABLE omop.tmp_dates_rows_condition AS
SELECT
    person_id AS person_id,
    condition_concept_id AS condition_concept_id,
    event_date AS event_date,
    ...
    -- this re-numbers the inner UNION so all rows are numbered ordered by the event date
FROM
    omop.tmp_dates_un_condition
;

DROP TABLE IF EXISTS omop.tmp_enddates_condition;
CREATE TABLE omop.tmp_enddates_condition AS
SELECT
    person_id AS person_id,
    condition_concept_id AS condition_concept_id,
    event_date - INTERVAL '30 day' AS end_date -- unpad the end date
FROM
    omop.tmp_dates_rows_condition e
WHERE
    (2 * e.start_ordinal) - e.overall_ord = 0
;

DROP TABLE IF EXISTS omop.tmp_conditionends;
CREATE TABLE omop.tmp_conditionends AS
SELECT
    c.person_id AS person_id,
    c.condition_concept_id AS condition_concept_id,
    c.condition_start_date AS condition_start_date,
    MIN(e.end_date) AS era_end_date
FROM
    omop.tmp_target_condition c
JOIN
    omop.tmp_enddates_condition e
    ON c.person_id = e.person_id
    AND c.condition_concept_id = e.condition_concept_id
    AND e.end_date >= c.condition_start_date
GROUP BY
...
DROP TABLE IF EXISTS omop.cdm_condition_era;
CREATE TABLE omop.cdm_condition_era
(
...
;
INSERT INTO omop.cdm_condition_era
SELECT
    ('x' || substr(md5(gen_random_uuid()::text), 1, 16))::bit(64)::bigint AS condition_era_id,
    person_id AS person_id,
    condition_concept_id AS condition_concept_id,
    MIN(condition_start_date) AS condition_era_start_date,
    era_end_date AS condition_era_end_date,
