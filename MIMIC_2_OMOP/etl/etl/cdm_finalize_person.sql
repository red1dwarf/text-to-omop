...
DROP TABLE IF EXISTS schema.tmp_person;
CREATE TABLE schema.tmp_person AS
SELECT per.*
FROM
    schema.cdm_person per
INNER JOIN
    schema.cdm_observation_period op
        ON per.person_id = op.person_id
;

TRUNCATE TABLE schema.cdm_person;

INSERT INTO schema.cdm_person
SELECT per.*
FROM
    schema.tmp_person per
;
