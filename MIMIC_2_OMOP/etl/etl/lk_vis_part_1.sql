...
DROP TABLE IF EXISTS omop.lk_admissions_clean;
CREATE TABLE omop.lk_admissions_clean AS
SELECT
    src.subject_id AS subject_id,
    src.hadm_id AS hadm_id,
    CASE 
...
DROP TABLE IF EXISTS omop.lk_transfers_clean;
CREATE TABLE omop.lk_transfers_clean AS
SELECT
    src.subject_id AS subject_id,
    COALESCE(src.hadm_id, vis.hadm_id) AS hadm_id,
    CAST(src.intime AS DATE) AS date_id,
    ...
    src.trace_id AS trace_id
FROM
    omop.src_transfers src
LEFT JOIN
    omop.lk_admissions_clean vis -- associate transfers with admissions according to 
        ON vis.subject_id = src.subject_id
        AND src.intime BETWEEN vis.start_datetime AND vis.end_datetime
        AND src.hadm_id IS NULL
LEFT JOIN
    omop.lk_admissions_clean pro
...
DROP TABLE IF EXISTS omop.lk_services_duplicated;
CREATE TABLE omop.lk_services_duplicated AS
SELECT
    trace_id, COUNT(*) AS row_count
FROM 
    omop.src_services src
GROUP BY
    src.trace_id
HAVING COUNT(*) > 1
;
...
DROP TABLE IF EXISTS omop.lk_services_clean;
CREATE TABLE omop.lk_services_clean AS
SELECT
    src.subject_id AS subject_id,
    src.hadm_id AS hadm_id,
    src.transfertime AS start_datetime,
...
FROM 
    omop.src_services src
LEFT JOIN
    omop.lk_services_duplicated sd
        ON src.trace_id = sd.trace_id
LEFT JOIN
    omop.lk_admissions_clean pro
        ON src.hadm_id = pro.hadm_id
