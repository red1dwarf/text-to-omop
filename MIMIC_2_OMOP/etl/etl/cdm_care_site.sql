...
DROP TABLE IF EXISTS omop.lk_trans_careunit_clean;
CREATE TABLE omop.lk_trans_careunit_clean AS
SELECT
    src.careunit              AS source_code,
    src.load_table_id         AS load_table_id,
    0                         AS load_row_id,
...
DROP TABLE IF EXISTS omop.cdm_care_site;
CREATE TABLE omop.cdm_care_site 
(
...
;
INSERT INTO omop.cdm_care_site
SELECT
    ('x' || substr(md5(gen_random_uuid()::text), 1, 16))::bit(64)::bigint AS care_site_id,
    src.source_code               AS care_site_name,
    vc2.concept_id                AS place_of_service_concept_id,
    1                             AS location_id,  -- hard-coded BIDMC
    src.source_code               AS care_site_source_value,
    ...
    src.trace_id                  AS trace_id
FROM 
    omop.lk_trans_careunit_clean src
LEFT JOIN
    omop.voc_concept vc
        ON vc.concept_code = src.source_code
        AND vc.vocabulary_id = 'mimiciv_cs_place_of_service' -- gcpt_care_site
