...
DROP TABLE IF EXISTS omop.tmp_drugIngredientExp;
CREATE TABLE omop.tmp_drugIngredientExp AS
SELECT
    de.drug_exposure_id AS drug_exposure_id,
    de.person_id AS person_id,
    de.drug_exposure_start_date AS drug_exposure_start_date,
    ...
    ds.denominator_unit_concept_id AS denominator_unit_concept_id,
    c.concept_class_id AS concept_class_id
FROM omop.cdm_drug_exposure de
INNER JOIN omop.voc_drug_strength ds
    ON de.drug_concept_id = ds.drug_concept_id
INNER JOIN omop.voc_concept_ancestor ca
    ON de.drug_concept_id = ca.descendant_concept_id
    AND ds.ingredient_concept_id = ca.ancestor_concept_id
LEFT JOIN omop.voc_concept c
...
DROP TABLE IF EXISTS omop.tmp_drugWithDose;
CREATE TABLE omop.tmp_drugWithDose AS
SELECT
    drug_exposure_id AS drug_exposure_id,
    person_id AS person_id,
    drug_exposure_start_date AS drug_exposure_start_date,
    ...
    CASE
        ...
        THEN
            ...
            END
    END AS unit_concept_id
FROM omop.tmp_drugIngredientExp
;



DROP TABLE IF EXISTS omop.tmp_cteDoseTarget;
CREATE TABLE omop.tmp_cteDoseTarget AS
SELECT
    dwd.drug_exposure_id AS drug_exposure_id,
    dwd.person_id AS person_id,
    dwd.drug_concept_id AS drug_concept_id,
    ...
    -- Add 1 day to the drug_exposure_start_date since
    -- there is no end_date or INTERVAL for the days_supply
FROM omop.tmp_drugWithDose dwd
WHERE 
    dose_value <> -1
;
...
DROP TABLE IF EXISTS omop.tmp_cteDoseEndDates_rawdata;
CREATE TABLE omop.tmp_cteDoseEndDates_rawdata AS
SELECT
    person_id AS person_id,
    drug_concept_id AS drug_concept_id,
    unit_concept_id AS unit_concept_id,
...
DROP TABLE IF EXISTS omop.tmp_cteDoseEndDates_e;
CREATE TABLE omop.tmp_cteDoseEndDates_e AS
SELECT
    person_id AS person_id,
    drug_concept_id AS drug_concept_id,
    unit_concept_id AS unit_concept_id,
    ...
    -- order by above pulls the current START down from the prior
    -- rows so that the NULLs from the END DATES will contain a value we can compare with
FROM omop.tmp_cteDoseEndDates_rawdata
;



DROP TABLE IF EXISTS omop.tmp_cteDoseEndDates;
CREATE TABLE omop.tmp_cteDoseEndDates AS
SELECT
    person_id AS person_id,
    drug_concept_id AS drug_concept_id,
    unit_concept_id AS unit_concept_id,
...
FROM omop.tmp_cteDoseEndDates_e
WHERE
    (2 * start_ordinal) - overall_ord = 0
;
...
DROP TABLE IF EXISTS omop.tmp_cteDoseFinalEnds;
CREATE TABLE omop.tmp_cteDoseFinalEnds AS
SELECT
    dt.person_id AS person_id,
    dt.drug_concept_id AS drug_concept_id,
    dt.unit_concept_id AS unit_concept_id,
    ...
    dt.drug_exposure_start_date AS drug_exposure_start_date,
    MIN(e.end_date) AS drug_era_end_date
FROM omop.tmp_cteDoseTarget dt
INNER JOIN omop.tmp_cteDoseEndDates e
    ON dt.person_id = e.person_id
    AND dt.drug_concept_id = e.drug_concept_id
    AND dt.unit_concept_id = e.unit_concept_id
    AND dt.dose_value = e.dose_value
...
DROP TABLE IF EXISTS omop.cdm_dose_era;
CREATE TABLE omop.cdm_dose_era (
...
INSERT INTO omop.cdm_dose_era
SELECT
    ('x' || substr(md5(gen_random_uuid()::text), 1, 16))::bit(64)::bigint AS dose_era_id,
    person_id AS person_id,
    drug_concept_id AS drug_concept_id,
    unit_concept_id AS unit_concept_id,
    dose_value AS dose_value,
    ...
    NULL AS load_row_id
FROM omop.tmp_cteDoseFinalEnds
GROUP BY
    person_id,
    drug_concept_id,
    unit_concept_id,
