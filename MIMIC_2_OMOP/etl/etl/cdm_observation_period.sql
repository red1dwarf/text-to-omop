...
DROP TABLE IF EXISTS omop.tmp_observation_period_clean;
CREATE TABLE omop.tmp_observation_period_clean AS
SELECT
    src.person_id AS person_id,
    MIN(src.visit_start_date) AS start_date,
    MAX(src.visit_end_date) AS end_date,
    src.unit_id AS unit_id
FROM
    omop.cdm_visit_occurrence src
GROUP BY
    src.person_id, src.unit_id
;

INSERT INTO omop.tmp_observation_period_clean
SELECT
    src.person_id AS person_id,
    MIN(src.condition_start_date) AS start_date,
    MAX(src.condition_end_date) AS end_date,
    src.unit_id AS unit_id
FROM
    omop.cdm_condition_occurrence src
GROUP BY
    src.person_id, src.unit_id
;
...
SELECT
    ...
    src.unit_id AS unit_id
FROM
    omop.cdm_procedure_occurrence src
GROUP BY
    src.person_id, src.unit_id
;
...
DROP TABLE IF EXISTS omop.tmp_observation_period;
CREATE TABLE omop.tmp_observation_period AS
SELECT
    src.person_id AS person_id,
    MIN(src.start_date) AS start_date,
    MAX(src.end_date) AS end_date,
    src.unit_id AS unit_id
FROM
    omop.tmp_observation_period_clean src
GROUP BY
    src.person_id, src.unit_id
;
...
DROP TABLE IF EXISTS omop.cdm_observation_period;
CREATE TABLE omop.cdm_observation_period 
(
...
INSERT INTO omop.cdm_observation_period
SELECT
    ('x' || substr(md5(gen_random_uuid()::text), 1, 16))::bit(64)::bigint AS observation_period_id,
    src.person_id AS person_id,
    MIN(src.start_date) AS observation_period_start_date,
    MAX(src.end_date) AS observation_period_end_date,
    32828 AS period_type_concept_id,  -- 32828    OMOP4976901 EHR episode record
    ...
    NULL AS trace_id
FROM
    omop.tmp_observation_period src
GROUP BY
    src.person_id
;
