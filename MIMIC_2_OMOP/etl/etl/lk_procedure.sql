...
DROP TABLE IF EXISTS omop.lk_datetimeevents_concept;
DROP TABLE IF EXISTS omop.lk_proc_event_clean;
DROP TABLE IF EXISTS omop.lk_datetimeevents_clean;



DROP TABLE IF EXISTS omop.lk_hcpcsevents_clean;
CREATE TABLE omop.lk_hcpcsevents_clean AS
 SELECT
    src.subject_id AS subject_id,
    src.hadm_id AS hadm_id,
    adm.dischtime AS start_datetime,
...
DROP TABLE IF EXISTS omop.lk_procedures_icd_clean;
CREATE TABLE omop.lk_procedures_icd_clean AS
SELECT
    src.subject_id AS subject_id,
    src.hadm_id AS hadm_id,
    adm.dischtime AS start_datetime,
...
DROP TABLE IF EXISTS omop.lk_proc_d_items_clean;
CREATE TABLE omop.lk_proc_d_items_clean AS
SELECT
    src.subject_id AS subject_id,
    src.hadm_id AS hadm_id,
    src.starttime AS start_datetime,
    ...
    src.trace_id AS trace_id
FROM
    omop.src_procedureevents src
;


INSERT INTO omop.lk_proc_d_items_clean
SELECT
    src.subject_id AS subject_id,
    src.hadm_id AS hadm_id,
    src.value AS start_datetime,
    ...
    src.trace_id AS trace_id    
FROM
    omop.src_datetimeevents src -- de
INNER JOIN
    omop.src_patients pat
        ON  pat.subject_id = src.subject_id
WHERE
    EXTRACT(YEAR FROM src.value) >= pat.anchor_year
;
...
DROP TABLE IF EXISTS omop.lk_hcpcs_concept;
CREATE TABLE omop.lk_hcpcs_concept AS
SELECT
    vc.concept_code AS source_code,
    vc.vocabulary_id AS source_vocabulary_id,
    vc.domain_id AS source_domain_id,
...
DROP TABLE IF EXISTS omop.lk_icd_proc_concept;
CREATE TABLE omop.lk_icd_proc_concept AS
SELECT
    REPLACE(vc.concept_code, '.', '') AS source_code,
    vc.vocabulary_id AS source_vocabulary_id,
    vc.domain_id AS source_domain_id,
...
DROP TABLE IF EXISTS omop.lk_itemid_concept;
CREATE TABLE omop.lk_itemid_concept AS
SELECT
    d_items.itemid AS itemid,
...
DROP TABLE IF EXISTS omop.lk_procedure_mapped;
CREATE TABLE omop.lk_procedure_mapped AS
SELECT
    src.subject_id AS subject_id, -- to person
    src.hadm_id AS hadm_id, -- to visit
    src.start_datetime AS start_datetime,
    ...
    src.trace_id AS trace_id
FROM
    omop.lk_hcpcsevents_clean src
LEFT JOIN
    omop.lk_hcpcs_concept lc
        ON src.hcpcs_cd = lc.source_code
;

INSERT INTO omop.lk_procedure_mapped
SELECT
    src.subject_id AS subject_id, -- to person
    src.hadm_id AS hadm_id, -- to visit
    src.start_datetime AS start_datetime,
    ...
    src.trace_id AS trace_id
FROM
    omop.lk_procedures_icd_clean src
LEFT JOIN
    omop.lk_icd_proc_concept lc
        ON src.source_code = lc.source_code
        AND src.source_vocabulary_id = lc.source_vocabulary_id
;
...
SELECT
    ...
    src.trace_id AS trace_id
FROM
    omop.lk_proc_d_items_clean src
LEFT JOIN
    omop.lk_itemid_concept lc
        ON src.itemid = lc.itemid
;