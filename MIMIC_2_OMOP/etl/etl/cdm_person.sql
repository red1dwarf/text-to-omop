...
DROP TABLE IF EXISTS omop.tmp_subject_ethnicity;
CREATE TABLE omop.tmp_subject_ethnicity AS
SELECT DISTINCT
    src.subject_id AS subject_id,
    FIRST_VALUE(src.ethnicity) OVER (
        PARTITION BY src.subject_id 
...
DROP TABLE IF EXISTS omop.lk_pat_ethnicity_concept;
CREATE TABLE omop.lk_pat_ethnicity_concept AS
SELECT DISTINCT
    src.ethnicity_first AS source_code,
    vc.concept_id AS source_concept_id,
    vc.vocabulary_id AS source_vocabulary_id,
    vc1.concept_id AS target_concept_id,
    vc1.vocabulary_id AS target_vocabulary_id  -- look here to distinguish Race and Ethnicity
FROM omop.tmp_subject_ethnicity src
LEFT JOIN
    -- gcpt_ethnicity_to_concept -> mimiciv_per_ethnicity
    omop.voc_concept vc
        ON UPPER(vc.concept_code) = UPPER(src.ethnicity_first) -- do the custom mapping
        AND vc.domain_id IN ('Race', 'Ethnicity')
LEFT JOIN
    omop.voc_concept_relationship cr1
        ON cr1.concept_id_1 = vc.concept_id
        AND cr1.relationship_id = 'Maps to'
LEFT JOIN
    omop.voc_concept vc1
        ON cr1.concept_id_2 = vc1.concept_id
        AND vc1.invalid_reason IS NULL
        AND vc1.standard_concept = 'S'
;
...
DROP TABLE IF EXISTS omop.cdm_person;
CREATE TABLE omop.cdm_person (
...
;
INSERT INTO omop.cdm_person
SELECT
    ('x' || substr(md5(gen_random_uuid()::text), 1, 16))::bit(64)::bigint AS person_id,
    CASE
        WHEN p.gender = 'F' THEN 8532  -- FEMALE
        WHEN p.gender = 'M' THEN 8507  -- MALE
        ELSE 0
...
FROM 
    omop.src_patients p
LEFT JOIN 
    omop.tmp_subject_ethnicity eth 
        ON p.subject_id = eth.subject_id
LEFT JOIN 
    omop.lk_pat_ethnicity_concept map_eth
        ON eth.ethnicity_first = map_eth.source_code
;
