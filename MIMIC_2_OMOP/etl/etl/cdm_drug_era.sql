...
DROP TABLE IF EXISTS omop.lk_join_voc_drug;
CREATE TABLE omop.lk_join_voc_drug AS
SELECT DISTINCT
    ca.descendant_concept_id AS descendant_concept_id,
    ca.ancestor_concept_id AS ancestor_concept_id,
    c.concept_id AS concept_id
FROM
    omop.voc_concept_ancestor ca
JOIN
    omop.voc_concept c
    ON ca.ancestor_concept_id = c.concept_id
    AND c.vocabulary_id IN ('RxNorm', 'RxNorm Extension')   -- selects RxNorm, RxNorm Extension vocabulary_id
...
DROP TABLE IF EXISTS omop.tmp_pretarget_drug;
CREATE TABLE omop.tmp_pretarget_drug AS
SELECT
    d.drug_exposure_id AS drug_exposure_id,
    d.person_id AS person_id,
    v.concept_id AS ingredient_concept_id,
    ...
    d.drug_exposure_end_date AS drug_exposure_end_date
FROM
    omop.cdm_drug_exposure d
JOIN
    omop.lk_join_voc_drug v
    ON v.descendant_concept_id = d.drug_concept_id
WHERE
    d.drug_concept_id != 0
;
...
DROP TABLE IF EXISTS omop.tmp_subenddates_un_drug;
CREATE TABLE omop.tmp_subenddates_un_drug AS
SELECT
    person_id AS person_id,
    ingredient_concept_id AS ingredient_concept_id,
    drug_exposure_start_date AS event_date,
...
DROP TABLE IF EXISTS omop.tmp_subenddates_rows_drug;
CREATE TABLE omop.tmp_subenddates_rows_drug AS
SELECT
    person_id AS person_id,
    ingredient_concept_id AS ingredient_concept_id,
    event_date AS event_date,
    ...
    ROW_NUMBER() OVER (
        ...
        ORDER BY event_date, event_type
            ...
            -- this re-numbers the inner UNION so all rows are numbered ordered by the event date
FROM
    omop.tmp_subenddates_un_drug
;



DROP TABLE IF EXISTS omop.tmp_subenddates_drug;
CREATE TABLE omop.tmp_subenddates_drug AS
SELECT
    person_id AS person_id,
    ingredient_concept_id AS ingredient_concept_id,
    event_date AS end_date
FROM
    omop.tmp_subenddates_rows_drug e
WHERE
    (2 * e.start_ordinal) - e.overall_ord = 0
;
...
DROP TABLE IF EXISTS omop.temp_ends_drug;
CREATE TABLE omop.temp_ends_drug AS
SELECT
    dt.person_id AS person_id,
    dt.ingredient_concept_id AS drug_concept_id,
    dt.drug_exposure_start_date AS drug_exposure_start_date,
    MIN(e.end_date) AS drug_sub_exposure_end_date
FROM
    omop.tmp_pretarget_drug dt
JOIN
    omop.tmp_subenddates_drug e
        ON dt.person_id = e.person_id
        AND dt.ingredient_concept_id = e.ingredient_concept_id
        AND e.end_date >= dt.drug_exposure_start_date
GROUP BY
...
DROP TABLE IF EXISTS omop.tmp_sub_drug;
CREATE TABLE omop.tmp_sub_drug AS
SELECT
    ROW_NUMBER() OVER (
...
DROP TABLE IF EXISTS omop.tmp_finaltarget_drug;
CREATE TABLE omop.tmp_finaltarget_drug AS
SELECT
    row_number AS row_number,
    person_id AS person_id,
    drug_concept_id AS ingredient_concept_id,
...
DROP TABLE IF EXISTS omop.tmp_enddates_un_drug;
CREATE TABLE omop.tmp_enddates_un_drug AS
SELECT
    person_id AS person_id,
    ingredient_concept_id AS ingredient_concept_id,
    drug_sub_exposure_start_date AS event_date,
...
DROP TABLE IF EXISTS omop.tmp_enddates_rows_drug;
CREATE TABLE omop.tmp_enddates_rows_drug AS
SELECT
    person_id AS person_id,
    ingredient_concept_id AS ingredient_concept_id,
    event_date AS event_date,
    ...
    -- this re-numbers the inner UNION so all rows are numbered ordered by the event date
FROM
    omop.tmp_enddates_un_drug
;



DROP TABLE IF EXISTS omop.tmp_enddates_drug;
CREATE TABLE omop.tmp_enddates_drug AS
SELECT
    person_id AS person_id,
    ingredient_concept_id AS ingredient_concept_id,
    event_date - INTERVAL '30 day' AS end_date  -- unpad the end date
FROM
    omop.tmp_enddates_rows_drug e
WHERE
    (2 * e.start_ordinal) - e.overall_ord = 0
;
...
DROP TABLE IF EXISTS omop.tmp_drugera_ends_drug;
CREATE TABLE omop.tmp_drugera_ends_drug AS
SELECT
    ft.person_id AS person_id,
    ft.ingredient_concept_id AS ingredient_concept_id,
    ft.drug_sub_exposure_start_date AS drug_sub_exposure_start_date,
    ...
    ft.days_exposed AS days_exposed
FROM
    omop.tmp_finaltarget_drug ft
JOIN
    omop.tmp_enddates_drug e
        ON ft.person_id = e.person_id
        AND e.end_date >= ft.drug_sub_exposure_start_date
        AND ft.ingredient_concept_id = e.ingredient_concept_id
GROUP BY
...
DROP TABLE IF EXISTS omop.cdm_drug_era;
CREATE TABLE omop.cdm_drug_era
(
...
INSERT INTO omop.cdm_drug_era
SELECT
    ('x' || substr(md5(gen_random_uuid()::text), 1, 16))::bit(64)::bigint AS drug_era_id,
    person_id AS person_id,
    ingredient_concept_id AS drug_concept_id,
    MIN(drug_sub_exposure_start_date) AS drug_era_start_date,
    drug_era_end_date AS drug_era_end_date,
