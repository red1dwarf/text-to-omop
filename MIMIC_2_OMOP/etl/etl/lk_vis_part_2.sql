...
DROP TABLE IF EXISTS omop.lk_visit_no_hadm_all;
CREATE TABLE omop.lk_visit_no_hadm_all AS
-- labevents
SELECT
    src.subject_id AS subject_id,
    CAST(src.start_datetime AS DATE) AS date_id,
    ...
    src.trace_id AS trace_id
FROM
    omop.lk_meas_labevents_mapped src
WHERE
    src.hadm_id IS NULL
UNION ALL
-- specimen
...
DROP TABLE IF EXISTS omop.lk_visit_no_hadm_dist;
CREATE TABLE omop.lk_visit_no_hadm_dist AS
SELECT
    src.subject_id AS subject_id,
    src.date_id AS date_id,
    MIN(src.start_datetime) AS start_datetime,
...
DROP TABLE IF EXISTS omop.lk_visit_clean;
CREATE TABLE omop.lk_visit_clean AS
SELECT
    ('x' || substr(md5(gen_random_uuid()::text), 1, 16))::bit(64)::bigint AS visit_occurrence_id,
    src.subject_id AS subject_id,
    src.hadm_id AS hadm_id,
    CAST(NULL AS DATE) AS date_id,
    src.start_datetime AS start_datetime,
    ...
    src.trace_id AS trace_id
FROM
    omop.lk_admissions_clean src -- adm
UNION ALL
SELECT
    ('x' || substr(md5(gen_random_uuid()::text), 1, 16))::bit(64)::bigint AS visit_occurrence_id,
    src.subject_id AS subject_id,
    ...
    src.trace_id AS trace_id
FROM
    omop.lk_visit_no_hadm_dist src -- adm
;



DROP TABLE IF EXISTS omop.lk_visit_detail_clean;
CREATE TABLE omop.lk_visit_detail_clean AS
SELECT
    ('x' || substr(md5(gen_random_uuid()::text), 1, 16))::bit(64)::bigint AS visit_detail_id,
    src.subject_id AS subject_id,
    src.hadm_id AS hadm_id,
    src.date_id AS date_id,
    src.start_datetime AS start_datetime,
...
INSERT INTO omop.lk_visit_detail_clean
SELECT
    ('x' || substr(md5(gen_random_uuid()::text), 1, 16))::bit(64)::bigint AS visit_detail_id,
    src.subject_id AS subject_id,
    src.hadm_id AS hadm_id,
...
DROP TABLE IF EXISTS omop.lk_visit_detail_prev_next;
CREATE TABLE omop.lk_visit_detail_prev_next AS
SELECT 
    src.visit_detail_id AS visit_detail_id,
    src.subject_id AS subject_id,
    src.hadm_id AS hadm_id,
    ...
    src.trace_id AS trace_id
FROM
    omop.lk_visit_detail_clean src
LEFT JOIN
    omop.lk_visit_clean vis
        ON src.subject_id = vis.subject_id
        AND (
            src.hadm_id = vis.hadm_id
            OR (src.hadm_id IS NULL AND src.date_id = vis.date_id)
...
DROP TABLE IF EXISTS omop.lk_visit_concept;
CREATE TABLE omop.lk_visit_concept AS
SELECT 
    vc.concept_code AS source_code,
    vc.concept_id AS source_concept_id,
    vc2.concept_id AS target_concept_id,
