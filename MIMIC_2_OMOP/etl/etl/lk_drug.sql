...
DROP TABLE IF EXISTS omop.lk_prescriptions_clean;
CREATE TABLE omop.lk_prescriptions_clean AS
SELECT
    -- -- 'drug:['                || COALESCE(drug, drug_name_poe, drug_name_generic,'') || ']'||
    -- 'drug:['                || COALESCE(drug,'') || ']'||
    -- 'prod_strength:['       || COALESCE(prod_strength,'') || ']'||
    ...
    'NDC' AS ndc_source_vocabulary,
    src.form_val_disp AS form_val_disp,
    CAST(SUBSTRING(src.form_val_disp FROM '[-]?[\d]+[.]?[\d]*') AS NUMERIC) AS quantity,
    -- COALESCE(
    --     -- src.drug, src.drug_name_poe, src.drug_name_generic,'')
    --     src.drug, '')
    --     || ' ' || COALESCE(src.prod_strength, '')
    TRIM(COALESCE(
        CASE WHEN src.drug IN ('Bag', 'Vial', 'Syringe', 'Syringe.',
                ...
                ELSE src.drug
            END, '') || ' ' || COALESCE(src.prod_strength, '')) AS gcpt_source_code,
    ...
    src.trace_id AS trace_id
FROM
    omop.src_prescriptions src -- pr
LEFT JOIN 
    omop.src_pharmacy pharm
        ON src.pharmacy_id = pharm.pharmacy_id
WHERE
    src.starttime IS NOT NULL
    AND src.drug IS NOT NULL
...
DROP TABLE IF EXISTS omop.lk_pr_ndc_concept;
CREATE TABLE omop.lk_pr_ndc_concept AS
SELECT DISTINCT
    src.ndc_source_code AS source_code,
    vc.domain_id AS source_domain_id,
    vc.concept_id AS source_concept_id,
    ...
    vc2.concept_id AS target_concept_id
FROM
    omop.lk_prescriptions_clean src -- pr
LEFT JOIN
    omop.voc_concept vc
        ON vc.concept_code = src.ndc_source_code --this covers 85% of direct mapping but no standard
        AND vc.vocabulary_id = src.ndc_source_vocabulary -- NDC
...
DROP TABLE IF EXISTS omop.lk_pr_gcpt_concept;
CREATE TABLE omop.lk_pr_gcpt_concept AS
SELECT DISTINCT
    src.gcpt_source_code AS source_code,
    vc.domain_id AS source_domain_id,
    vc.concept_id AS source_concept_id,
...
DROP TABLE IF EXISTS omop.lk_pr_route_concept;
CREATE TABLE omop.lk_pr_route_concept AS
SELECT DISTINCT
    src.route_source_code AS source_code,
    vc.domain_id AS source_domain_id,
    vc.concept_id AS source_concept_id,
...
UPDATE omop.voc_concept_relationship
SET concept_id_2 = 40486069
WHERE concept_id_1 = 2000002030;
SET concept_id_2 = 40486069
WHERE concept_id_1 = 2000002007;
SET concept_id_2 = 40486069
WHERE concept_id_1 = 2000002016;
SET concept_id_2 = 40486069
WHERE concept_id_1 = 2000002039;
SET concept_id_2 = 35624178
WHERE concept_id_1 = 2000002021;
SET concept_id_2 = 35624178
WHERE concept_id_1 = 2000002093;
SET concept_id_2 = 37174549
WHERE concept_id_1 = 2000002075;


DROP TABLE IF EXISTS omop.lk_drug_mapped;
CREATE TABLE omop.lk_drug_mapped AS
SELECT
    src.hadm_id AS hadm_id,
    src.subject_id AS subject_id,
    COALESCE(vc_ndc.target_concept_id, vc_gcpt.target_concept_id, 0) AS target_concept_id,
    ...
    COALESCE(mu.source_concept_id,0) AS unit_source_concept_id,
    COALESCE(mu.target_concept_id,0) AS unit_concept_id,
    --
    CONCAT('drug.', src.unit_id) AS unit_id,
    src.load_table_id AS load_table_id,
    src.load_row_id AS load_row_id,
    src.trace_id AS trace_id
FROM
    omop.lk_prescriptions_clean src
LEFT JOIN
    omop.lk_pr_ndc_concept vc_ndc
        ON src.ndc_source_code = vc_ndc.source_code
        AND vc_ndc.target_concept_id IS NOT NULL
LEFT JOIN
    omop.lk_pr_gcpt_concept vc_gcpt
        ON src.gcpt_source_code = vc_gcpt.source_code
        AND vc_gcpt.target_concept_id IS NOT NULL
LEFT JOIN
    omop.lk_pr_route_concept vc_route
        ON src.route_source_code = vc_route.source_code
        AND vc_route.target_concept_id IS NOT NULL
LEFT JOIN 
    omop.lk_meas_unit_concept mu
        ON mu.source_code = src.dose_unit_source_code
        AND mu.target_concept_id IS NOT NULL
;