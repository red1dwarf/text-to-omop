...
CREATE TABLE schema.d_items_to_concept AS
WITH counts AS (
    -- d_items for chartevents
    SELECT
        -- itemid
        ...
        COUNT(*) AS row_count
    FROM 
        schema.lk_chartevents_mapped src
    GROUP BY
        src.itemid,
        src.source_label,
        src.source_vocabulary_id,
    ...
    SELECT
        ...
        src.target_concept_id AS target_concept_id,
        COUNT(*) AS row_count
    FROM schema.lk_procedure_mapped src
    WHERE src.unit_id LIKE '%.procedureevents%' OR src.unit_id LIKE '%.datetimeevents%'
    GROUP BY
        src.itemid,
        src.source_label,
        src.source_vocabulary_id,
...
FROM 
    counts src
LEFT JOIN 
    schema.voc_concept vc
        ON src.source_concept_id = vc.concept_id
LEFT JOIN 
    schema.voc_concept vc2
        ON src.target_concept_id = vc2.concept_id
ORDER BY
    itemid, target_vocabulary_id, target_concept_id
;
...
CREATE TABLE schema.d_labitems_to_concept AS
WITH counts AS (
    SELECT
        itemid AS itemid,
        COUNT(*) AS row_count
    FROM 
        schema.lk_meas_labevents_mapped
    GROUP BY itemid
)
SELECT
    -- itemid
    ...
    counts.row_count AS row_count
FROM 
    schema.lk_meas_d_labitems_concept src
LEFT JOIN
    counts 
        USING (itemid)
ORDER BY
...
CREATE TABLE schema.d_micro_to_concept AS
WITH counts AS (
    SELECT
        spec_itemid AS itemid,
        COUNT(*) AS row_count
    FROM 
        schema.lk_specimen_mapped
    GROUP BY itemid
    UNION ALL -- more unions
    SELECT
        ...
        COUNT(*) AS row_count
    FROM 
        schema.lk_meas_organism_mapped
    GROUP BY itemid
    UNION ALL -- more unions
    SELECT
...
SELECT
    ...
    src.target_concept_id AS target_concept_id,
    src.target_concept_name AS target_concept_name,
    src.target_standard_concept AS target_standard_concept,
    counts.row_count AS row_count
FROM 
    schema.lk_d_micro_concept src
LEFT JOIN 
    counts 
        USING (itemid)
LEFT JOIN 
