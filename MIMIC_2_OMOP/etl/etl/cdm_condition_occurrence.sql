...
DROP TABLE IF EXISTS omop.cdm_condition_occurrence;
CREATE TABLE omop.cdm_condition_occurrence (
    ...
    condition_source_value VARCHAR(60) NULL,
...
INSERT INTO omop.cdm_condition_occurrence
SELECT
    ('x' || substr(md5(gen_random_uuid()::text), 1, 16))::bit(64)::bigint AS condition_occurrence_id,
    per.person_id AS person_id,
    COALESCE(src.target_concept_id, 0) AS condition_concept_id,
    CAST(src.start_datetime AS DATE) AS condition_start_date,
    src.start_datetime AS condition_start_datetime,
...
SELECT
    ...
    src.trace_id AS trace_id
FROM
    omop.lk_chartevents_condition_mapped src
INNER JOIN
    omop.cdm_person per
        ON CAST(src.subject_id AS TEXT) = per.person_source_value
INNER JOIN
