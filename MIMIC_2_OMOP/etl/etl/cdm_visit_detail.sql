...
DROP TABLE IF EXISTS omop.cdm_visit_detail;
CREATE TABLE omop.cdm_visit_detail 
(
...
INSERT INTO omop.cdm_visit_detail
SELECT
    src.visit_detail_id AS visit_detail_id,
    per.person_id AS person_id,
    COALESCE(vdc.target_concept_id, 0) AS visit_detail_concept_id,
    ...
    src.trace_id AS trace_id
FROM
    omop.lk_visit_detail_prev_next src
INNER JOIN
    omop.cdm_person per
...
LEFT JOIN
    omop.cdm_provider prov
        ON src.provider_id = prov.provider_source_value
;