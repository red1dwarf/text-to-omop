...
DROP TABLE IF EXISTS omop.lk_meas_d_labitems_clean;
CREATE TABLE omop.lk_meas_d_labitems_clean AS
SELECT
    dlab.itemid AS itemid, -- for <cdm>.<source_value>
...
DROP TABLE IF EXISTS omop.lk_meas_labevents_clean;
CREATE TABLE omop.lk_meas_labevents_clean AS
SELECT
...
DROP TABLE IF EXISTS omop.lk_meas_d_labitems_concept;
CREATE TABLE omop.lk_meas_d_labitems_concept AS
SELECT
    dlab.itemid AS itemid,
    dlab.source_code AS source_code,
    dlab.loinc_code AS loinc_code,
    ...
    vc2.standard_concept AS target_standard_concept
FROM
    omop.lk_meas_d_labitems_clean dlab
LEFT JOIN
    omop.voc_concept vc
...
DROP TABLE IF EXISTS omop.lk_meas_labevents_hadm_id;
CREATE TABLE omop.lk_meas_labevents_hadm_id AS
SELECT
    src.trace_id AS event_trace_id, 
    adm.hadm_id AS hadm_id,
    ROW_NUMBER() OVER (
...
FROM  
    omop.lk_meas_labevents_clean src
INNER JOIN 
...
DROP TABLE IF EXISTS omop.lk_meas_labevents_mapped;
CREATE TABLE omop.lk_meas_labevents_mapped AS
SELECT
    src.measurement_id AS measurement_id,
    src.subject_id AS subject_id,
    COALESCE(src.hadm_id, hadm.hadm_id) AS hadm_id,
...
FROM  
    omop.lk_meas_labevents_clean src
INNER JOIN 
    omop.lk_meas_d_labitems_concept labc
        ON labc.itemid = src.itemid
LEFT JOIN 
...
LEFT JOIN 
    omop.lk_meas_unit_concept uc
        ON uc.source_code = src.valueuom
LEFT JOIN 
    omop.lk_meas_labevents_hadm_id hadm
        ON hadm.event_trace_id = src.trace_id
        AND hadm.row_num = 1
;