...
DROP TABLE IF EXISTS omop.lk_diagnoses_icd_clean;
CREATE TABLE omop.lk_diagnoses_icd_clean AS
SELECT
    src.subject_id AS subject_id,
    src.hadm_id AS hadm_id,
    CASE
...
DROP TABLE IF EXISTS omop.lk_diagnoses_icd_mapped;
CREATE TABLE omop.lk_diagnoses_icd_mapped AS
SELECT
    src.subject_id AS subject_id,
    src.hadm_id AS hadm_id,
    src.seq_num AS seq_num,
    ...
    src.trace_id AS trace_id
FROM
    omop.lk_diagnoses_icd_clean src
LEFT JOIN
    omop.voc_concept vc
        ON REPLACE(vc.concept_code, '.', '') = REPLACE(TRIM(src.source_code), '.', '')
        AND vc.vocabulary_id = src.source_vocabulary_id
...
DROP TABLE IF EXISTS omop.tmp_seq_num_to_concept;